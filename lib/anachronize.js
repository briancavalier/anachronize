...
function anachronize(options, globs) {
	var pkg, tmpl, name, main, nsSep, excludes,
		transform, writeFile;
	...
	transform = fn.compose(
		...
		apply(generateModule(tmpl.module))
	);
	...
	return [
		...
		mergeOutput(tmpl.main, main, nsSep),
		writeFile
	].reduce(when, globs);
}
...
function mergeOutput(template, main, nsSep) {
...
function generateModule(template) {
