var fs, path, glob, when, fn, nodefn, minimatch, template,
	jsFilesRx, readFile, extractModuleNameRx, fakeRequire, defineTemplate;
...
fakeRequire = 'function require(id){ var m = __root__;id = id.split(\'.\');while(m && id.length>0) { m = m[id.shift()]; } return m; };';
defineTemplate = template.bind(null, 'function(factory){__root__.{{global}}=factory(require);}');
...
function anachronize(options, globs) {
	var pkg, mainTemplate, name, main, ns, excludes,
		transform, writeFile;
